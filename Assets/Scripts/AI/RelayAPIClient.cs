...
namespace MasterCheff.AI
{
    ...
    public class RelayAPIClient : Singleton<RelayAPIClient>
    {
        ...
        [SerializeField] private string _imageEndpoint = "/generate-image";
        
        [Header("Timeouts")]
        [SerializeField] private int _judgeTimeoutSeconds = 30;
        [SerializeField] private int _imageTimeoutSeconds = 60;
        ...
        /// </summary>
        public async Task<ImageGenerationResponse> RequestImageGeneration(ImageGenerationRequest request)
        {
            string url = $"{_baseUrl}{_imageEndpoint}";
            string jsonBody = JsonUtility.ToJson(request);

            Debug.Log($"[RelayAPIClient] Sending image generation request to {url}");
            OnRequestStarted?.Invoke("image");

            try
            {
                string response = await PostJsonAsync(url, jsonBody, _imageTimeoutSeconds);
                
                if (!string.IsNullOrEmpty(response))
                {
                    var result = JsonUtility.FromJson<ImageGenerationResponse>(response);
                    OnRequestCompleted?.Invoke("image");
                    return result;
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"[RelayAPIClient] Image generation request failed: {ex.Message}");
                OnRequestFailed?.Invoke("image", ex.Message);
            }
        ...
        public async Task<Texture2D> DownloadImage(string imageUrl)
        {
            ...
            {
                Debug.LogWarning("[RelayAPIClient] Image URL is empty");
                return null;
            }

            Debug.Log($"[RelayAPIClient] Downloading image from {imageUrl}");

            try
            {
                using (UnityWebRequest request = UnityWebRequestTexture.GetTexture(imageUrl))
                {
                    request.timeout = _imageTimeoutSeconds;
                    ...
                    {
                        Debug.LogError($"[RelayAPIClient] Image download failed: {request.error}");
                    }
                }
            }
            catch (Exception ex)
            {
                Debug.LogError($"[RelayAPIClient] Image download exception: {ex.Message}");
            }
        ...
        /// </summary>
        public void Configure(string baseUrl, string judgeEndpoint = null, string imageEndpoint = null)
        {
            ...
            if (!string.IsNullOrEmpty(imageEndpoint))
            {
                _imageEndpoint = imageEndpoint.StartsWith("/") ? imageEndpoint : "/" + imageEndpoint;
            }

            Debug.Log($"[RelayAPIClient] Configured - Base: {_baseUrl}, Judge: {_judgeEndpoint}, Image: {_imageEndpoint}");
        }
